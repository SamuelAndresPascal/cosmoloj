...
/**
 * <div>EPSG::9826</div>
 * <div class="en">Lambert Conic Conformal (1SP West Orientated)</div>
 *
 * @author Samuel Andrés
 */
@Reference(Cosmoloj.IOGP_GUIDANCE_NOTE_7_2_2019)
public class Epsg9826 implements InvertibleProjection {
    ...
    public Epsg9826(final Ellipsoid ellipsoid, final double phi0, final double lambda0, final double k0,
    ...
    public static Epsg9826 ofParams(final Ellipsoid ellipsoid, final Map<MethodParameter, ?> params) {
        return new Epsg9826(ellipsoid,
    ...
    public double[] compute(final double[] input) {
        ...
        return new double[]{westing(phi, lambda), northing(phi, lambda)};
    }
    ...
    public double[] inverse(final double[] input) {
        ...
        return new double[]{phi(westing, northing), lambda(westing, northing)};
    }

    double westing(final double phi, final double lambda) {
    ...
    double lambda(final double westing, final double northing) {
        return invTheta(westing, northing) / n + lambda0;
    }

    double phi(final double westing, final double northing) {
        double phi = invT(westing, northing);

        while (true) {
            double tmp = phi(westing, northing, phi);
    ...
    double phi(final double westing, final double northing, final double phi) {
        return Math.PI / 2. - 2. * Math.atan(
                invT(westing, northing) * Math.pow((1. - e * Math.sin(phi)) / (1. + e * Math.sin(phi)), e / 2.)
        );
    }

    double invTheta(final double westing, final double northing) {
        return Math.atan2(fe - westing, r0 - (northing - fn));
    }

    double invT(final double westing, final double northing) {
        return Math.pow(invR(westing, northing) / (a * f), 1. / n);
    }

    double invR(final double westing, final double northing) {
        final double relWesting = fe - westing;
        final double relNorthing = r0 - (northing - fn);
        final double result = Math.sqrt(relWesting * relWesting + relNorthing * relNorthing);
