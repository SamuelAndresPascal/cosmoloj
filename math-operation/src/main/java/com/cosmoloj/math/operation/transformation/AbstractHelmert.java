...
/**
 * <div class="en">Helmert 7-parameter transformations</div>
 *
 * @author Samuel Andrés
 */
@Cite(Cosmoloj.IOGP_GUIDANCE_NOTE_7_2_2019)
abstract class AbstractHelmert implements InversibleCoordinateTransformation, AutoInverse<double[]> {
    ...
    protected AbstractHelmert(final double tx, final double ty, final double tz,
            final double rx, final double ry, final double rz,
            final double ds, final boolean exactMatrices, final boolean exactReverse) {
    ...
    public double[] compute(final double[] input) {
        return DoubleTabulars.add(DoubleTabulars.external(1. + ds, DoubleTabulars.mult(r, input)), t);
    }
    ...
    public double[] inverse(final double[] input) {
        if (exactReverse) {
            return DoubleTabulars.minus(
                DoubleTabulars.external(1. / (1. + ds), DoubleTabulars.mult(rr, input)),
                    DoubleTabulars.external(1. / (1. + ds), DoubleTabulars.mult(rr, t)));
        } else {
            return DoubleTabulars.minus(
                DoubleTabulars.external(1. - ds, DoubleTabulars.mult(rr, input)), t);
        }
    }
    ...
    public Object getParameter(final MethodParameter parameter) {
        return switch (parameter) {
            ...
            case X_AXIS_ROTATION -> r[0];
            case Y_AXIS_ROTATION -> r[1];
            case Z_AXIS_ROTATION -> r[2];
