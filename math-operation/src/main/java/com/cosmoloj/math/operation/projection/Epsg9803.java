...
/**
 * <div>EPSG::9803</div>
 * <div class="en">Lambert Conic Conformal (2SP Belgium)</div>
 *
 * @author Samuel Andrés
 */
@Cite(Cosmoloj.IOGP_GUIDANCE_NOTE_7_2_2019)
public class Epsg9803 implements InvertibleProjection {
    ...
    public static final double SEMI_MAJOR = 29.2985 / 3600. / 360. * (2. * Math.PI);
    ...
    public Epsg9803(final Ellipsoid ellipsoid, final double phif, final double lambdaf, final double phi1,
    ...
    public static Epsg9803 ofParams(final Ellipsoid ellipsoid, final Map<MethodParameter, ?> params) {
        return new Epsg9803(ellipsoid,
    ...
    double easting(final double phi, final double lambda) {
        return ef + r(phi) * Math.sin(theta(lambda) - SEMI_MAJOR);
    }

    double northing(final double phi, final double lambda) {
        return nf + rf - r(phi) * Math.cos(theta(lambda) - SEMI_MAJOR);
    }
    ...
    double lambda(final double easting, final double northing) {
        return (invTheta(easting, northing) + SEMI_MAJOR) / n + lambdaf;
    }
