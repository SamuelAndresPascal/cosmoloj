 ...
 */
public interface ConstraintLastPredicateTokenBuilder<I extends Token>
        extends ConstraintPredicateTokenBuilder<I> {

    @Override
    default Predicate<? super I> constraint(final int historyIndex) {
        return before(historyIndex) == 1 ? constraintLast(tokens().size()) : t -> true;
    }

    /**
     *
     * @param currentIndex <span class="fr">index du jeton en cours de test</span>
     * @return <span class="fr">le prédicat à appliquer au dernier jeton de l'historique</span>
     */
    Predicate<? super I> constraintLast(int currentIndex);
}
